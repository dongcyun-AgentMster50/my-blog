...
| 정답 | `/^\s*(?:([IVXLC]{1,5})-)?(\d{1,3})\.\s+the answers? (is|are)\s+([A-E](?:\s*(?:,|and|&)\s*[A-E])*)\b/i` → 섹션·번호·정답 글자들. 해설은 그 문단부터 다음 정답 문단 직전까지의 문단(페이지 경계 포함) |
...
- `pages` 크기 `[수정 2026-09-20 — 초판 추정이 틀렸다]`: **아이템 배열은 저장하지 않는다**(그건 그대로 유효하다).
  ...
  **`[미확정]` 이 수치는 `JSON.stringify` 바이트다. IndexedDB는 구조화 복제(structured clone)라
