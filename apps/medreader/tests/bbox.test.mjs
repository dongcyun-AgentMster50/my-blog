...
test('B1 폭을 가진 공백 아이템이 x1 을 밀지 않는다 (목차 점선 자리)', () => {
  ...
  //
  // `[수정 2026-09-23 — 6단계 Review]` 이 픽스처는 **자기 이름의 규칙을 검증하지
  // 못했다.** 원래 공백이 112+400=512 에서 끝나고 페이지 번호가 512+10=522 에서
  // 끝나, 공백을 넣든 빼든 x1 이 522 로 같았다 — 공백 제외 가드를 통째로 없애도
  // 통과했다. 실제 결함은 **공백이 마지막 글자보다 더 오른쪽까지 뻗을 때** 나므로
  // 그렇게 고쳤다.
  const bb = lineBBox(lineOfItems([
    it0('Chapter', 72, 700, 40, { idx: 0 }),   // → 112
    it0('   ', 112, 700, 460, { idx: 1 }),     // → 572. 폭이 깨져 페이지 번호를 넘어간다
    it0('37', 500, 700, 10, { idx: 2 })        // → 510. 이것이 줄의 진짜 오른쪽 끝이다
  ]));
  assert.equal(bb.x0, 72);
  assert.equal(bb.x1, 510, 'x1 이 공백 아이템 때문에 밀렸다');
});
