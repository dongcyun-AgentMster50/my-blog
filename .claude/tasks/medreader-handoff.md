...
## 이월 결함 — 10b Review 가 찾았으나 범위 밖이라 두고 온 것 (2026-09-25)

1. **`showOriginal` 경합 — 원본 뷰 첫 진입에서 렌더가 중간에 버려진다.**
   `showOriginal` 이 겹쳐 불려 `paint()` 가 `renderPageInto` 직후 토큰 검사에서
   빠져나간다. 증상: "원본 쪽을 여는 중…" 문구가 남고, 줌 % 라벨이 비고,
   `canvas[data-scale]` 이 안 붙고, **`state.viewport` 가 null 이라 낭독 하이라이트
   오버레이가 안 뜬다.** `[−][+]` 를 한 번 누르면 정상으로 돌아온다.
   **10b 가 만든 것이 아니다** — HEAD(`a67e923`)에서 재현 확인됨.
   → 사용자에게 보이는 결함이다. 다음 단계에서 우선순위 높게 볼 것.

2. ~~**`refitOriginal()` 을 아무도 부르지 않는다(죽은 코드).**~~ → **2026-09-25 해결** (`watchResize`)
   `resize`·`orientationchange` 구독이 `js/` 전체에 **0건**이라,
   12-3 의 "화면 회전·리사이즈 시 하이라이트 재계산(원본 뷰는 viewport 재생성)"이
   실제로는 돌지 않는다. 폴더블은 접었다 펴는 기기다 — 실기기에서 드러날 것이다.

3. **`.typeset-panel`·`.tts-rate-panel` 의 `max-block-size` 가 14절 배너 높이를 빼지 않는다.**
   배너가 떠 있으면 팝오버가 상단바를 덮는다(실측: 패널 top 73 vs 상단바 bottom 130).
   10b 이전에도 같았고 악화되지 않았다.

4. **spec 12-3·16-G 의 "재생 버튼 64px" 표기가 코드(56px)와 어긋난다.**
   10b 에서 세로 예산을 위해 줄였고 16-G 의 48px 하한은 지킨다. spec 갱신이 필요하다.

---

## 2차 실기기 확인 결과 (2026-09-25 저녁, Z Fold 7)

| | 결과 |
|---|---|
| 하이라이트 정합(10a) | ○ **해결** |
| 원본 뷰 스크롤 소유권(10b) | ○ **해결** — 확대해도 안쪽만 움직이고 끝에서 체이닝 없음 |
| `showOriginal` 경합(이월 ①) | ○ **사용자는 안 겪음** |
| 접었다 펴기 — 하이라이트 위치 | ○ **유지된다** (canvas 와 오버레이가 같은 좌표계라 함께 스케일됨) |

**새로 나온 것 둘 — 둘 다 고침:**

1. **접었을 때 줌이 자동으로 안 맞았다.** 사용자: "폴드를 편 상태에서 이 앱을 열면
   120% 였었어. 접었을때는 자동으로 맞춰지진 않아서, 75%까지 축소했어(그게 한계야)".
   → 이월 ②(`refitOriginal` 죽은 코드)가 원인이었다. `watchResize()` 로 연결했다
   (`resize`·`orientationchange`·`visualViewport.resize`, 150ms 디바운스).
   사용자가 줌을 직접 건드렸으면 그 선택을 존중하고 오버레이만 다시 잡는다.

2. **퀴즈 칩을 "나중에"로 닫으면 돌아갈 길이 없었다.** 사용자: "나중에 눌러서
   닫혀버렸어 어떻게 켜는지 몰러. (안보임)" — 칩이 **유일한 입구**였다.
   문구가 거짓말을 한 셈이다.
   → 상단바에 **닫을 수 없는 [퀴즈] 버튼**을 더했다. 칩과 **같은 판정**
   (`chipEntry`)을 쓰되, 버튼이 따로 판정하지 않고 칩이 정한 결과를
   `onQuizEntry` 로 받는다 — 규칙이 두 벌이 되면 어긋난다.

   그 과정에서 **원본 뷰에서는 칩도 버튼도 아예 안 뜨는 것**을 발견했다.
   `paintQuizChip()` 이 `notifyPage()` 에만 매달려 있었는데, 원본 뷰는 이월 ①의
   경합으로 렌더가 버려지면 `notifyPage()` 까지 건너뛴다. 퀴즈 판정은 `docId` 와
   `page` 만 있으면 되므로 **버려질 수 있는 비동기 작업 앞에서** 한 번 그리게 했다.
