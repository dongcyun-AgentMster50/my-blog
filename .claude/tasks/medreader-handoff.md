...
# 현재 상태 (2026-09-25 종료 시점)

**커밋 `b5fa6f9` 까지 배포됨.** 테스트 **459개 전부 통과**.
...
|---|---|
| 1~5 (+5b) | 완료·배포 |
| 6 (원본 뷰·오버레이·표 크롭) | 완료·배포 (6a bbox / 6b 뷰 / Review 반영) |
| 9 (퀴즈) | 완료·배포 (9a 파서 / 9b 엔진·화면) |
| 10 부분 | 10a 하이라이트 정합 · 10b 바·스크롤 · 10c 리사이즈·퀴즈 입구 · 10d 정답 되찾기 · 10e 출제 정책 |
| **7 (AI 프로바이더·키 화면)** | **미착수** |
| **8 (번역·요약·사용량)** | **미착수** |
| **10 나머지** | manifest · 접근성 마감 · **16절 전수 점검** |

`algoVersion` **8** · `storeVersion` **1** · pdf.js **5.4.149 고정**
`tests/review6.test.mjs` 의 V9 가 `algoVersion` 을 고정한다 — 올리려면 그 테스트도 같이 고쳐야 한다.

## 파서 현황 (729쪽 실측)

```
섹션 14 (로마숫자 10 + 폴백 4) · 문항 1,233 · verified 1,185 (96.1%) · unverified 48
퀴즈 칩이 뜨는 섹션 10개 (0.6 임계 && 5문항 이상)
```
**채점 불가 문항은 출제하지 않는다**(`planQuizOrder`). 그래서 unverified 48 은 사용자에게 안 보인다.

## 다음에 볼 만한 것 (우선순위 순)

1. **실기기 3차 확인** — 사용자가 2026-09-26 에 볼 예정. 확인 항목은 아래 "미확인" 참조.
2. **7단계(AI)** — 키 설정 화면이 있어야 8단계가 의미를 갖는다.
3. 남은 unverified 48 중 고칠 수 있는 것:
   - `sec-III#137`·`sec-X#95` — **보기가 A·B 두 개만 추출**됐다(정답은 C·E). 정답이 아니라
     **보기 추출이 잘린 것**이다. `parser.js` 의 보기 이어붙이기가 두 문단 이상 건너뛰는 경우를 놓친다.
   - `I-58`·`I-59`·`III-64`~`67` — **보기를 공유하는 matching 세트**라 문항 자체가
     `rejectedNoOptions`(153건)로 걸러진다. 5-3 에 형식 규정이 없다.
   - 폴백 섹션 4개(`sec-p293`·`p471`·`p549`·`p680`)는 각 1문항·전부 unverified 인 오탐이다.
4. **이월 결함** — 위 "이월 결함" 절 참조. `showOriginal` 경합(①)은 사용자가 안 겪는다고 확인됐다.

## 사용자가 아직 확인 안 한 것 (2026-09-26 예정)

- 퀴즈 1번 문항에 **정답과 해설이 나오는가** (10d 의 목적)
- 칩 문항 수가 **실제 낼 수**로 보이는가
- **뒤로가기가 읽던 쪽**으로 가는가
- 퀴즈 전반 — 채점·[오답만 다시]·이어 풀기

## 일하는 방식 (사용자 확인됨)

- **진행 내역·도구 설명·서브에이전트 보고를 전부 한국어로.** 사용자가 보는 표면이 그것이다.
- Build 와 Review 서브에이전트를 분리하고, 지침 파일(`.claude/tasks/*.md`)을 만들어 넘긴다.
- **서브에이전트는 커밋하지 않는다.** 오케스트레이터가 직접 검증하고 커밋한다.
- 에이전트 보고를 그대로 믿지 말 것 — **독립 스크립트로 재측정**한다. 실제로 두 번
  틀린 보고가 있었다(“F 정답 0건”이 실은 3건, “변이 테스트 통과”가 실은 죽은 코드).
- **변이 테스트를 돌린다.** 규칙을 일부러 깨서 빨개지는 테스트가 없으면 그 테스트는 헛돈다.
- 막히면 우회하지 말고 멈춰서 보고한다(사용자 메모리 규칙).

---

## 3차 실기기 확인 (2026-09-26, Z Fold 7) — 전부 해결

| 확인 항목 | 결과 |
|---|---|
| 하이라이트가 문장에 맞는가 (10a) | ○ |
| 접은 화면에서 원본 뷰 잘림 (10b) | ○ — 75%로 줄이면 한 장에 다 보임 |
| 스크롤 소유권 (10b) | ○ — 확대해도 안쪽만, 끝에서 체이닝 없음 |
| `showOriginal` 경합 | ○ 사용자는 안 겪음 |
| 접었다 펴기 — 하이라이트 위치 | ○ 유지 |
| 접었다 펴기 — 줌 자동 재맞춤 (10c) | ○ |
| **퀴즈가 1번부터 시작, 정답 나옴 (10d+10f)** | **○** ← `parserVersion` 경첩이 실제로 작동 |

### 이 라운드에서 배운 것 (다음 세션이 알아야 할 것)

1. **파서를 고치면 `parserVersion` 을 올려야 한다.** `js/quiz/parser.js` 의 상수다.
   `algoVersion`(추출 경첩)과 따로다 — 파서만 바뀌었는데 7000쪽을 다시 뽑을 이유가 없다.
   이걸 안 올려서 10d 가 정답 31개를 되찾고도 사용자 기기에서는 아무 변화가 없었다.
   **판본 경첩이 없으면 고친 것이 사용자에게 도달하지 않는다.**

2. **GitHub Pages 는 파일마다 캐시 만료가 달라 "반쪽 상태"가 생긴다.**
   새 `index.html` + 옛 `quiz.js` 같은 조합이 실제로 나온다. 그래서 화면 요소의
   동작을 **한 가지 장치**로 몰아야 한다(`data-nav` 전역 위임 하나. JS `onclick` 과
   섞으면 둘 중 하나만 낡아도 죽은 버튼이 된다).

3. **로컬 검증에서도 같은 캐시에 두 번 데었다.** `python -m http.server` 는
   `Cache-Control` 을 안 보낸다. `location.reload()` 로는 안 풀린다 —
   고친 파일 전부를 `fetch(url, {cache:'reload'})` 로 다시 받은 뒤 쿼리를 바꿔 navigate 할 것.

4. **UI 는 "주 동작이 어디서 일어나는가"를 따라간다.** 문항 이동을 카드 아래에
   뒀더니 "넘기려고 스크롤해야 한다"는 지적을 받았다. 훑어보기가 주 동작이면
   이동은 손이 이미 가 있는 곳(상단바)에 있어야 한다.
